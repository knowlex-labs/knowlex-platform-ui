...
export function BlogAdminList() {
  ...
  return (
    <div>
      ...
      {error && (
        <div className="rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-900 mb-4">
          {error}
        </div>
      )}

      <div className="rounded-xl border border-ledger-200 bg-white overflow-hidden">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-ledger-100">
            {isLoading ? (
              ...
              blogs.map(blog => (
                <tr key={blog.id} className="hover:bg-ledger-50">
